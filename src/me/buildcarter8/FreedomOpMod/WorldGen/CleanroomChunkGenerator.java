 ...
 */
package me.buildcarter8.FreedomOpMod.WorldGen;
...
public class CleanroomChunkGenerator extends ChunkGenerator
{
    ...
    public CleanroomChunkGenerator(String id)
    {
        if (id != null)
        {
            try
            {
                ...
                if (id.length() > 0)
                {
                    ...
                    for (int i = 0; i < tokens.length; i += 2)
                    {
                        ...
                        if (height <= 0)
                        {
                            FOPMLog.warning("[CleanroomGenerator] Invalid height '" + tokens[i] + "'. Using 64 instead.");
                            height = 64;
                        }
                        ...
                        if (materialTokens.length == 2)
                        {
                            ...
                            {
                                FOPMLog.warning("[CleanroomGenerator] Invalid Data Value '" + materialTokens[1] + "'. Defaulting to 0.");
                                dataValue = 0;
                            }
                        }
                        ...
                        if (mat == null)
                        {
                            ...
                            if (mat == null)
                            {
                                FOPMLog.warning("[CleanroomGenerator] Invalid Block ID '" + materialTokens[0] + "'. Defaulting to stone.");
                                mat = Material.STONE;
                            }
                        }
                        ...
                        if (!mat.isBlock())
                        {
                            FOPMLog.warning("[CleanroomGenerator] Error, '" + materialTokens[0] + "' is not a block. Defaulting to stone.");
                            mat = Material.STONE;
                        }
            ...
            {
                FOPMLog.severe("[CleanroomGenerator] Error parsing CleanroomGenerator ID '" + id + "'. using defaults '64,1': " + e.toString());
                layerDataValues = null;
                layer = new short[65];
    ...
    public short[][] generateExtBlockSections(World world, Random random, int x, int z, ChunkGenerator.BiomeGrid biomes)
    {
        ...
        {
            FOPMLog.warning("[CleanroomGenerator] Error, chunk height " + layer.length + " is greater than the world max height (" + maxHeight + "). Trimming to world max height.");
